...
public abstract class AbstractServerSocketAppender<E> extends AppenderBase<E> {
	...
	public void start() {
		...
		try {
			final ServerSocket socket = getServerSocketFactory().createServerSocket(getPort(), getBacklog(), getInetAddress());
			final ServerListener<RemoteReceiverClient> listener = createServerListener(socket);
	...
	protected void append(final E event) {
		...
		runner.accept(client -> client.offer(serEvent));
	}
