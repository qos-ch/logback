...
public class SSLContextFactoryBean {
    ...
    public SSLContext createContext(final ContextAware context) throws NoSuchProviderException, NoSuchAlgorithmException, KeyManagementException,
        ...
        final KeyManager[] keyManagers = createKeyManagers(context);
        final TrustManager[] trustManagers = createTrustManagers(context);
        final SecureRandom secureRandom = createSecureRandom(context);
    ...
    private TrustManager[] createTrustManagers(final ContextAware context) throws NoSuchProviderException, NoSuchAlgorithmException, KeyStoreException {
        ...
        final KeyStore trustStore = getTrustStore().createKeyStore();
        context.addInfo("trust store of type '" + trustStore.getType() + "' provider '" + trustStore.getProvider() + "': " + getTrustStore().getLocation());

        final TrustManagerFactory tmf = getTrustManagerFactory().createTrustManagerFactory();
    ...
    private SecureRandom createSecureRandom(final ContextAware context) throws NoSuchProviderException, NoSuchAlgorithmException {
        ...
        final SecureRandom secureRandom = getSecureRandom().createSecureRandom();
