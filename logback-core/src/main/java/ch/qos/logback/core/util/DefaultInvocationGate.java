...
public class DefaultInvocationGate implements InvocationGate {
    ...
    private final long minDelayThreshold;
    private final long maxDelayThreshold;
     ...
     */
    @Override
    final public boolean isTooSoon(final long currentTime) {
        final boolean maskMatch = (invocationCounter++ & mask) == mask;
        ...
        } else if (currentTime > upperLimitForNoMaskMatch) {
            decreaseMask();
            updateLimits(currentTime);
            return false;
        }
