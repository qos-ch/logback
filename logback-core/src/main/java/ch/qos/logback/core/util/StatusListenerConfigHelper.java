...
public class StatusListenerConfigHelper {
    ...
    public static void installIfAsked(final Context context) {
        final String slClass = OptionHelper.getSystemProperty(CoreConstants.STATUS_LISTENER_CLASS_KEY);
    ...
    private static void initAndAddListener(final Context context, final StatusListener listener) {
        if (listener != null) {
            ...
            final boolean effectivelyAdded = context.getStatusManager().add(listener);
     ...
     * @since 1.0.1
     */
    static public void addOnConsoleListenerInstance(final Context context, final OnConsoleStatusListener onConsoleStatusListener) {
