...
public abstract class AbstractSocketAppender<E> extends AppenderBase<E> implements SocketConnector.ExceptionHandler {
    ...
    public void start() {
        ...
        if (errorCount == 0) {
            ...
            task = getContext().getScheduledExecutorService().submit(() -> connectSocketAndDispatchEvents());
            super.start();
        }
    }
    ...
    private void connectSocketAndDispatchEvents() {
        try {
            while (socketConnectionCouldBeEstablished()) {
                try {
                    final ObjectWriter objectWriter = createObjectWriterForSocket();
    ...
    private ObjectWriter createObjectWriterForSocket() throws IOException {
        ...
        final ObjectWriter objectWriter = objectWriterFactory.newAutoFlushingObjectWriter(socket.getOutputStream());
    ...
    private SocketConnector createConnector(final InetAddress address, final int port, final int initialDelay, final long retryDelay) {
        final SocketConnector connector = newConnector(address, port, initialDelay, retryDelay);
    ...
    private void dispatchEvents(final ObjectWriter objectWriter) throws InterruptedException, IOException {
        while (true) {
            final E event = deque.takeFirst();
            postProcessEvent(event);
            final Serializable serializableEvent = getPST().transform(event);
            try {
