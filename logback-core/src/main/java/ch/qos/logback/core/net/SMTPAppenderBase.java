...
public abstract class SMTPAppenderBase<E> extends AppenderBase<E> {
    ...
    private final List<PatternLayoutBase<E>> toPatternLayoutList = new ArrayList<>();
    private String from;
    ...
    private Session lookupSessionInJNDI() {
        ...
        try {
            ...
            final Object obj = initialContext.lookup(jndiLocation);
            return (Session) obj;
    ...
    protected void append(final E eventObject) {
        ...
        try {
            if (eventEvaluator.evaluate(eventObject)) {
                ...
                final CyclicBuffer<E> cbClone = new CyclicBuffer<>(cb);
    ...
    private List<InternetAddress> parseAddress(final E event) {
        final int len = toPatternLayoutList.size();

        final List<InternetAddress> iaList = new ArrayList<>();

        for (int i = 0; i < len; i++) {
            try {
                final PatternLayoutBase<E> emailPL = toPatternLayoutList.get(i);
     ...
     */
    protected void updateMimeMsg(final MimeMessage mimeMsg, final CyclicBuffer<E> cb, final E lastEventObject) {
    }
    ...
    protected void sendBuffer(final CyclicBuffer<E> cb, final E lastEventObject) {
        ...
        try {
            ...
            if (subjectLayout != null) {
                ...
                // new-line character.
                final int newLinePos = subjectStr != null ? subjectStr.indexOf('\n') : -1;
                if (newLinePos > -1) {
            ...
            final List<InternetAddress> destinationAddresses = parseAddress(lastEventObject);
    ...
    public void addTo(final String to) {
        ...
        final PatternLayoutBase<E> plb = makeNewToPatternLayout(to.trim());
    ...
    public List<String> getToAsListOfString() {
        ...
        for (final PatternLayoutBase<E> plb : toPatternLayoutList) {
