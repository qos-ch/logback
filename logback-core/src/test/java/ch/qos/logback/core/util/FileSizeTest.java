...
public class FileSizeTest {
    ...
    public void testValueOf() {
        {
            final FileSize fs = FileSize.valueOf("8");
            assertEquals(8, fs.getSize());
        }

        {
            final FileSize fs = FileSize.valueOf("8 kbs");
            assertEquals(8 * KB_CO, fs.getSize());
        }

        {
            final FileSize fs = FileSize.valueOf("8 kb");
            assertEquals(8 * KB_CO, fs.getSize());
        }

        {
            final FileSize fs = FileSize.valueOf("12 mb");
            assertEquals(12 * MB_CO, fs.getSize());
        }

        {
            final FileSize fs = FileSize.valueOf("5 GBs");
    ...
    public void testToString() {
        {
            final FileSize fs = new FileSize(8);
