...
public class PropertyEvalScriptBuilderTest {
	...
	void buildAndAssertTrue(final String scriptStr) throws Exception {
		final Condition condition = pesb.build(scriptStr);
		assertNotNull(condition);
		assertTrue(condition.evaluate());
	}

	void buildAndAssertFalse(final String scriptStr) throws Exception {
		final Condition condition = pesb.build(scriptStr);
		assertNotNull(condition);
