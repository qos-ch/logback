...
public class HardenedObjectInputStreamTest {
    ...
    public void smoke() throws ClassNotFoundException, IOException {
        final Innocent innocent = new Innocent();
        innocent.setAnInt(1);
        innocent.setAnInteger(2);
        innocent.setaString("smoke");
        final Innocent back = writeAndRead(innocent);
        assertEquals(innocent, back);
    }

    private Innocent writeAndRead(final Innocent innocent) throws IOException, ClassNotFoundException {
        writeObject(oos, innocent);
        final ByteArrayInputStream bis = new ByteArrayInputStream(bos.toByteArray());
        inputStream = new HardenedObjectInputStream(bis, whitelist);
        final Innocent fooBack = (Innocent) inputStream.readObject();
        inputStream.close();
        return fooBack;
    }
