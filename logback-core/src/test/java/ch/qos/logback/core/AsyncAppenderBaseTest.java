...
//		assertTrue(delayingListAppender.interrupted);
	...
	public void eventLossIfNeverBlock() {
		...
		delayingListAppender.setDelay(5); // (loopLen*delay) much bigger than
											// test timeout
		asyncAppenderBase.addAppender(delayingListAppender);
		asyncAppenderBase.setQueueSize(bufferSize);
		...
		asyncAppenderBase.stop();
		// ListAppender size isn't a reliable test here, so just make sure we
		// didn't
...
//		assertFalse(asyncAppenderBase.isStarted());
	...
	public void stopExitsWhenMaxRuntimeReached() throws InterruptedException {
		int maxFlushTime = 1; // runtime of 0 means wait forever, so use 1 ms
								// instead
		int loopLen = 10;
		ListAppender<Integer> la = delayingListAppender;
		asyncAppenderBase.addAppender(la);
	...
	// consumed by async appender. See also
	// http://jira.qos.ch/browse/LOGBACK-910
	@Test
...
//		statusChecker.assertContainsMatch("Worker thread will flush remaining events before exiting.");
	...
	public void checkThatStartMethodIsIdempotent() {
		...
		// we don't need mockito for this test, but if we did here is how it
		// would look
		// AsyncAppenderBase<Integer> spied = Mockito.spy(asyncAppenderBase);
		// Mockito.doThrow(new
		// IllegalStateException("non idempotent start")).when((UnsynchronizedAppenderBase<Integer>)
		// spied).start();

		// a second invocation of start will cause a IllegalThreadStateException
		// thrown by the asyncAppenderBase.worker
		// thread
		asyncAppenderBase.start();
	}
