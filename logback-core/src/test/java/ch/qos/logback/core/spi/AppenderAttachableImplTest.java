...
public class AppenderAttachableImplTest {
    ...
    public void testAddAppender() throws Exception {
        ...
        ta.start();
        aai.addAppender(ta);
        final int size = aai.appendLoopOnAppenders(event);
        assertTrue("Incorrect number of appenders", size == 2);
    }

    @Test
    public void testIteratorForAppenders() throws Exception {
        final NOPAppender<TestEvent> ta = new NOPAppender<>();
        ta.start();
        aai.addAppender(ta);
        final NOPAppender<TestEvent> tab = new NOPAppender<>();
        tab.setName("test");
        tab.start();
        aai.addAppender(tab);
        final Iterator<Appender<TestEvent>> iter = aai.iteratorForAppenders();
        int size = 0;
        while (iter.hasNext()) {
            ++size;
            final Appender<TestEvent> app = iter.next();
    ...
    public void getGetAppender() throws Exception {
        final NOPAppender<TestEvent> test = new NOPAppender<>();
        test.setName("test");
        test.start();
        aai.addAppender(test);

        final NOPAppender<TestEvent> testOther = new NOPAppender<>();
