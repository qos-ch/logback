...
public class TimeBasedRollingTest extends ScaffoldingForRollingTests {
    ...
    void initRFA(final RollingFileAppender<Object> rfa, final String filename) {
    ...
    void genericTest(final String testId, final String patternPrefix, final String compressionSuffix, final UnaryOperator<String> filenameFunction, final int waitDuration) throws IOException {
        ...
        final String fileNamePatternStr = randomOutputDir + patternPrefix + "-%d{" + DATE_PATTERN_WITH_SECONDS + "}" + compressionSuffix;
    ...
    void doRestart(final String testId, final String patternPart, final UnaryOperator<String> filenameFunction, final int waitDuration) {
        // change the timestamp of the currently actively file
        final File activeFile = new File(rfa1.getFile());
        activeFile.setLastModified(currentTime);

        incCurrentTime(waitDuration);

        final String filePatternStr = randomOutputDir + patternPart + "-%d{" + DATE_PATTERN_WITH_SECONDS + "}";
    ...
    public void failed_rename() throws IOException {
        ...
        final String testId = "failed_rename";
        FileOutputStream fos = null;
        try {
            final String fileName = testId2FileName(testId);
