...
/**
 * Implicitly tests configurable max pool size of {@link ch.qos.logback.core.net.server.AbstractServerSocketAppender} and
 * that each additional client connection is being dropped.
 *
 * @author Sebastian Gr&ouml;bler
 */
public class ServerSocketAppenderExecutorServiceIntegrationTest {
    ...
    private final ConnectionDropCountingServerSocketAppender socketAppender = new ConnectionDropCountingServerSocketAppender(dropCounter);

    @Before
    public void beforeTest() {
        socketAppender.setContext(context);
        socketAppender.setAddress(HOST);
        socketAppender.setPort(PORT);
        socketAppender.setMaxPoolSize(MAX_POOL_SIZE);
    ...
    public void dropsClientsWhenMaxPoolSizeIsReached() throws InterruptedException {
        ...
        for (int i = 0; i < CLIENT_COUNT; i++) {
            final SocketReceiver socketReceiver = new SocketReceiver();
            socketReceiver.setContext(context);
            socketReceiver.setRemoteHost(HOST);
            socketReceiver.setPort(PORT);
