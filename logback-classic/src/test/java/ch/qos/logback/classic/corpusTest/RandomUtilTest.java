...
public class RandomUtilTest {
	...
	public void smoke() {
		...
		final int EXPECTED_AVERAGE = 6;
		final int EXPECTED_STD_DEVIATION = 3;

		System.out.println();
		final Random r = new Random(now);
		final int len = 3000;
		final int[] valArray = new int[len];
		for (int i = 0; i < len; i++) {
			valArray[i] = RandomUtil.gaussianAsPositiveInt(r, EXPECTED_AVERAGE, EXPECTED_STD_DEVIATION);
		}
		final double avg = average(valArray);

		assertEquals(EXPECTED_AVERAGE, avg, 0.3);
	}

	public double average(final int[] va) {
		double avg = 0;
