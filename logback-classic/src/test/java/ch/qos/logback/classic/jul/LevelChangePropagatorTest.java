...
public class LevelChangePropagatorTest {
	...
	void checkLevelChange(final String loggerName, final Level level) {
	...
	@Test
	public void root() {
		checkLevelChange(org.slf4j.Logger.ROOT_LOGGER_NAME, Level.TRACE);
	}

	// see http://jira.qos.ch/browse/LBCLASSIC-256
	@Test
	public void gc() {
		final Logger logger = loggerContext.getLogger("gc" + rand);
		logger.setLevel(Level.INFO);
		// invoke GC so that the relevant julLogger can be garbage collected.
		System.gc();
		final java.util.logging.Logger julLogger = JULHelper.asJULLogger(logger);
		final java.util.logging.Level julLevel = JULHelper.asJULLevel(Level.INFO);
