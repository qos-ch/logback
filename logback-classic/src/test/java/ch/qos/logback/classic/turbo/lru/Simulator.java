...
public class Simulator {
    ...
    public Simulator(final int worldSize, final int get2PutRatio, final boolean multiThreaded) {
    ...
    public List<Event<String>> generateScenario(final int len) {
        final List<Event<String>> scenario = new ArrayList<>();
        ...
        for (int i = 0; i < len; i++) {
            ...
            final Event<String> e = new Event<>(put, String.valueOf(r));
            scenario.add(e);
        }
        return scenario;
    }

    public void simulate(final List<Event<String>> scenario, final LRUCache<String, String> lruCache, final T_LRUCache<String> tlruCache) {
        for (final Event<String> e : scenario) {
            if (e.put) {
                lruCache.put(e.k, e.k);
                tlruCache.put(e.k);
            } else {
                final String r0 = lruCache.get(e.k);
                final String r1 = tlruCache.get(e.k);
                if (!multiThreaded) {
