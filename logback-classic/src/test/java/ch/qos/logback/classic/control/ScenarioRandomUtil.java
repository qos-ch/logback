...
public class ScenarioRandomUtil {
	...
	public static boolean oneInFreq(final int freq) {
		return random.nextInt(freq) % freq == 0;
	}

	public static Level randomLevel() {
		final int rl = random.nextInt(6);
		switch (rl) {
		case 0:
			return null;
		case 1:
	...
	public static String randomLoggerName(final int average, final int stdDeviation) {
		final int depth = RandomUtil.gaussianAsPositiveInt(random, average, stdDeviation);
	...
	public static String randomId() {
		...
		final int len = RandomUtil.gaussianAsPositiveInt(random, AVERAGE_ID_LEN, AVERAGE_ID_DEV);
		final StringBuilder buf = new StringBuilder();
		for (int i = 0; i < len; i++) {
			final int offset = random.nextInt(26);
	 ...
	 * @return
	 */
	public static int randomChildrenCount(final String name) {
		final int dots = dotCount(name);
		if (dots <= 1) {
			return 3;
		}
		if (dots == 2 || dots == 3) {
			return 9;
		} else if (shouldHaveChildrenWithProbabilitz(0.5)) {
	 ...
	 * @return
	 */
	static boolean shouldHaveChildrenWithProbabilitz(final double p) {
		if (p < 0 || p > 1.0) {
	...
	static int dotCount(final String s) {
		int count = 0;
