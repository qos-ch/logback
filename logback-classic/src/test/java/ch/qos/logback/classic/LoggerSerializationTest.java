...
public class LoggerSerializationTest {
    ...
    public void basicSerialization() throws IOException, ClassNotFoundException {
        final Foo foo = new Foo(logger);
        foo.doFoo();
        final Foo fooBack = writeAndRead(foo);
        fooBack.doFoo();
    }
    ...
    public void deepTreeSerialization() throws IOException {
        ...
        final Logger a = lc.getLogger("aaaaaaaa");
        ...
        final Logger b = lc.getLogger("b");

        writeObject(oos, a);
        oos.close();
        final int sizeA = bos.size();
    ...
    Foo readFooObject(final HardenedObjectInputStream inputStream) throws IOException, ClassNotFoundException {
        return (Foo) readObject(inputStream);
    }

    private Object readObject(final HardenedObjectInputStream inputStream) throws IOException, ClassNotFoundException {
        return inputStream.readObject();
    }

    private void writeObject(final ObjectOutputStream oos, final Object o) throws IOException {
        oos.writeObject(o);
        oos.flush();
        oos.close();
    }
