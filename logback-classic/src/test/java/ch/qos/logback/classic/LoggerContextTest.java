...
public class LoggerContextTest {
	...
	public void testDotDot() {
		final Logger dotdot = lc.getLogger("..");
	...
	public void testLoggerXY() {
		...
		final Logger xy = lc.getLogger("x.y");
		...
		final Logger xy2 = lc.getLogger("x.y");
		assertEquals(xy, xy2);

		final Logger x2 = lc.getLogger("x");
		assertEquals(x, x2);
		assertEquals(3, instanceCount());
	}
	...
	public void testLoggerMultipleChildren() {
		...
		final Logger xy0 = lc.getLogger("x.y0");
		LoggerTestHelper.assertNameEquals(xy0, "x.y0");

		final Logger xy1 = lc.getLogger("x.y1");
		...
		for (int i = 0; i < 100; i++) {
			final Logger xy_i = lc.getLogger("x.y" + i);
	...
	public void resetTest() {
		...
		final Logger root = lc.getLogger(org.slf4j.Logger.ROOT_LOGGER_NAME);
		final Logger a = lc.getLogger("a");
		final Logger ab = lc.getLogger("a.b");
	...
	public void turboFilterStopOnReset() {
		final NOPTurboFilter nopTF = new NOPTurboFilter();
		nopTF.start();
	...
	public void loggerNameEndingInDotOrDollarShouldWork() {
		{
			final String loggerName = "toto.x.";
	...
	public void collisionMapsPostReset() {
		...
		final Map<String, String> fileCollisions = (Map<String, String>) lc.getObject(FA_FILENAME_COLLISION_MAP);
