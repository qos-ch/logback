...
public class SocketReceiver extends ReceiverBase implements Runnable, SocketConnector.ExceptionHandler {
	...
	public void run() {
		try {
			final LoggerContext lc = (LoggerContext) getContext();
			while (!Thread.currentThread().isInterrupted()) {
				final SocketConnector connector = createConnector(address, port, 0, reconnectionDelay);
	...
	private Socket waitForConnectorToReturnASocket() throws InterruptedException {
		try {
			final Socket s = connectorTask.get();
			connectorTask = null;
			return s;
