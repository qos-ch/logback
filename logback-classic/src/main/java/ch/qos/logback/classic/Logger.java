...
public final class Logger implements org.slf4j.Logger, LocationAwareLogger, AppenderAttachable<ILoggingEvent>, Serializable {
	...
	Logger getChildByName(final String childName) {
		...
		final int len = childrenList.size();
		for (int i = 0; i < len; i++) {
	...
	public synchronized void setLevel(Level newLevel) {
		...
		if (childrenList != null) {
			final int len = childrenList.size();
			for (int i = 0; i < len; i++) {
				final Logger child = childrenList.get(i);
	...
	Logger createChildByLastNamePart(final String lastPart) {
		...
		if (isRootLogger()) {
			...
			childLogger = new Logger(name + CoreConstants.DOT + lastPart, this, loggerContext);
		}
		childrenList.add(childLogger);
		childLogger.effectiveLevelInt = effectiveLevelInt;
		return childLogger;
	}
	...
	void recursiveReset() {
		...
		for (final Logger childLogger : childrenList) {
	...
	Logger createChildByName(final String childName) {
		...
		Logger childLogger;
		childLogger = new Logger(childName, this, loggerContext);
		childrenList.add(childLogger);
		childLogger.effectiveLevelInt = effectiveLevelInt;
		return childLogger;
	}
	...
	public boolean isInfoEnabled(final Marker marker) {
		final FilterReply decision = callTurboFilters(marker, Level.INFO);
		if (decision == FilterReply.NEUTRAL) {
	...
	public boolean isErrorEnabled(final Marker marker) {
		final FilterReply decision = callTurboFilters(marker, Level.ERROR);
		if (decision == FilterReply.NEUTRAL) {
	...
	public boolean isWarnEnabled(final Marker marker) {
		final FilterReply decision = callTurboFilters(marker, Level.WARN);
		if (decision == FilterReply.NEUTRAL) {
	...
	public boolean isEnabledFor(final Marker marker, final Level level) {
		final FilterReply decision = callTurboFilters(marker, level);
		if (decision == FilterReply.NEUTRAL) {
	...
	public void log(final org.slf4j.event.LoggingEvent slf4jEvent) {
		final org.slf4j.event.Level slf4jLevel = slf4jEvent.getLevel();
		final Level logbackLevel = Level.convertAnSLF4JLevel(slf4jLevel);
