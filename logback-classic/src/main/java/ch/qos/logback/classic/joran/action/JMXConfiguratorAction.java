...
public class JMXConfiguratorAction extends Action {
    ...
    public void begin(final InterpretationContext ec, final String name, final Attributes attributes) throws ActionException {
        ...
        final String contextNameAttributeVal = attributes.getValue(CONTEXT_NAME_ATTRIBUTE_NAME);
        if (!OptionHelper.isNullOrEmpty(contextNameAttributeVal)) {
            contextName = contextNameAttributeVal;
        }

        String objectNameAsStr;
        final String objectNameAttributeVal = attributes.getValue(OBJECT_NAME_ATTRIBUTE_NAME);
        ...
        final ObjectName objectName = MBeanUtil.string2ObjectName(context, this, objectNameAsStr);
        if (objectName == null) {
        ...
        if (!MBeanUtil.isRegistered(mbs, objectName)) {
            ...
            // unregisters the
            final JMXConfigurator jmxConfigurator = new JMXConfigurator((LoggerContext) context, mbs, objectName);
            try {
