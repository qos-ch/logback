...
public class ConfigurationDelegate extends ContextAwareBase {
	...
	void scan(final String scanPeriodStr = null) {
		if (scanPeriodStr) {
	...
	void statusListener(final Class listenerClass) {
		final StatusListener statusListener = listenerClass.newInstance()
	...
	void conversionRule(final String conversionWord, final Class converterClass) {
	...
	void root(final Level level, final List<String> appenderNames = []) {
		if (level == null) {
	...
	void logger(final String name, final Level level, final List<String> appenderNames = [], Boolean additivity = null) {
		if (name) {
	...
	void appender(final String name, final Class clazz, final Closure closure = null) {
	...
	void receiver(final String name, final Class aClass, final Closure closure = null) {
	...
	void turboFilter(final Class clazz, final Closure closure = null) {
	...
	String timestamp(final String datePattern, final long timeReference = -1) {
		long now = -1;
	 ...
	 */
	void jmxConfigurator(final String name = null) {
		def objectName = null
