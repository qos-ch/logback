...
public class DBAppenderHSQLTest {
	...
	int existingEventTableRowCount(final Statement stmt) throws SQLException {
		final ResultSet rs = stmt.executeQuery("SELECT count(*) FROM access_event");
		int result = -1;
		if (rs.next()) {
			result = rs.getInt(1);
		}
	...
	private void setInsertHeadersAndStart(final boolean insert) {
	...
	public void testAppendAccessEvent() throws SQLException {
		...
		final IAccessEvent event = createAccessEvent();
	...
	public void testAppendMultipleEvents() throws SQLException {
		setInsertHeadersAndStart(false);
		final String uri = "testAppendMultipleEvents";
		for (int i = 0; i < 10; i++) {
			final IAccessEvent event = createAccessEvent(uri);
			appender.append(event);
		}
	...
	private IAccessEvent createAccessEvent(final String uri) {
		final DummyRequest request = new DummyRequest();
		request.setRequestUri(uri);
		final DummyResponse response = new DummyResponse();
		final DummyServerAdapter adapter = new DummyServerAdapter(request, response);
